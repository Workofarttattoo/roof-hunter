...
@dataclass
class DijkstraGraphShortestPathConfig:
    """Configuration for Dijkstra Graph Shortest Path"""
    iterations: int = 1000
    precision: float = 1e-6

class DijkstraGraphShortestPathLab:
    """
    Dijkstra Graph Shortest Path
    ...
    def __init__(self, config: DijkstraGraphShortestPathConfig):
        self.config = config
        self.results = []
...
def demo():
    """Demo of Dijkstra Graph Shortest Path"""
    print(f"{lab_name} - Built by ECH0")
    print("="*60)

    config = DijkstraGraphShortestPathConfig(iterations=500)
    lab = DijkstraGraphShortestPathLab(config)
