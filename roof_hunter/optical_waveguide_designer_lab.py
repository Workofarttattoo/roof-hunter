...
@dataclass
class OpticalWaveguideDesignerConfig:
    """Configuration for Optical Waveguide Designer"""
    iterations: int = 1000
    precision: float = 1e-6

class OpticalWaveguideDesignerLab:
    """
    Optical Waveguide Designer
    ...
    def __init__(self, config: OpticalWaveguideDesignerConfig):
        self.config = config
        self.results = []
...
def demo():
    """Demo of Optical Waveguide Designer"""
    print(f"{lab_name} - Built by ECH0")
    print("="*60)

    config = OpticalWaveguideDesignerConfig(iterations=500)
    lab = OpticalWaveguideDesignerLab(config)
