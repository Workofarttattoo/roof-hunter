...
function App() {
  ...
  const scrollToLeads = () => {
    ...
    setTimeout(() => {
      document.getElementById('marketplace-leads')?.scrollIntoView({ behavior: 'smooth' });
    }, 60);
  };
  ...
  const scrollToAgent = () => {
    setActiveTab('marketplace');
    setTimeout(() => {
      document.getElementById('sales-agent')?.scrollIntoView({ behavior: 'smooth' });
    }, 60);
  };
    ...
    <div className="ridgeline-app ridgeline-pro">
        ...
        {activeTab === 'marketplace' && (
          <section className="marketplace-section fade-in">
            <LandingHero
              ...
              onSignIn={openSignIn}
              onTalkToAgent={scrollToAgent}
            />

            <BentoFeatures onBrowseLeads={scrollToLeads} />

            <div className="pipeline-stats-card glass-premium">
              <div className="pipeline-stats-head">
                <h3 className="pipeline-stats-title">Forensic pipeline · live counts</h3>
              ...
              <p className="hero-filter-meta">
                ...
                <button type="button" className="linkish inline-link" onClick={openSignIn}>
                  Sign in to pull full unlocks
                </button>
              </p>
            </div>
            ...
            <HailStrikeMap
              ...
              onScrollToLeads={scrollToLeads}
            />

            <section className="map-property-preview glass-inset" aria-label="Street-level property map">
              <div className="pipeline-stats-head">
                <h3 className="pipeline-stats-title">Street-level map (OpenStreetMap)</h3>
                <p className="muted small" style={{ margin: 0, maxWidth: '42rem' }}>
                  Demo marker at <strong>35.4676, -97.5164</strong> (zoom {18}). Connect{' '}
                  <code className="inline-code">GET /imagery</code> or <code className="inline-code">POST /property</code>{' '}
                  to drive center / marker from your API.
                </p>
              </div>
              <MapView />
            </section>

            <div id="sales-agent" className="sales-agent-panel glass-inset">
              <div className="sales-agent-panel-head">
                <Phone size={18} className="sales-agent-panel-icon" aria-hidden />
                <div>
                  <h3 className="sales-agent-panel-title">Roof Hunter sales agent</h3>
                  <p className="muted small sales-agent-panel-sub">
                    Voice demo (ElevenLabs): subscriptions, individual leads, exclusive territory — same agent as your
                    dashboard “roof hunter sales agent” deployment.
                  </p>
                </div>
              </div>
              <SalesAgentWidget />
            ...
            <div className="damage-infographic glass-inset">
              <div className="info-graphics-head">
                ...
                <div>
                  <h3 className="info-graphics-title">Leads + live hail alerts</h3>
                  <p className="info-graphics-desc">
                    <strong>High</strong> ≥40 · <strong>Medium</strong> 15–39 · <strong>Low</strong> &lt;15 (inventory) plus{' '}
                    <strong>NWS</strong> active alerts that mention hail (
                    <code className="inline-code">api.weather.gov/alerts/active</code>). Bar shares 100% across all four
                    slices.
                  </p>
                </div>
              </div>
              {metricsTier?.summary ? (
                <>
                  <div
                    ...
                    role="img"
                    aria-label="Lead damage tiers and NWS hail-related active alerts"
                  >
                    {(metricsTier.lead_storm_pie?.slices || []).map((s) => {
                      if ((s.pct || 0) <= 0) return null;
                      const segClass =
                        s.key === 'lead_high'
                          ? 'high'
                          : s.key === 'lead_medium'
                      ...
                      return (
                        <div
                          key={s.key}
                          className={`tier-seg ${segClass}`}
                          style={{ flex: Math.max(s.pct || 0, 0.35) }}
                  ...
                  <ul className="tier-legend">
                    {(metricsTier.lead_storm_pie?.slices || []).map((s) => {
                      ...
                      return (
                        <li key={s.key}>
                          <span className={`tier-dot ${dotClass}`} /> {s.label}{' '}
                          <strong>{s.value}</strong> <span className="pct">({s.pct}%)</span>
                        </li>
                      );
                    })}
                    <li className="tier-total">Inventory total {metricsTier.summary.total} contacts</li>
                  </ul>
                  {metricsTier.nws_hail_alerts?.error && (
                    <p className="muted small" role="status">
                      NWS alerts unavailable: {metricsTier.nws_hail_alerts.error}
                    </p>
                  )}
                </>
              ) : (
              ...
              <div className="damage-infographic-cta">
                <button type="button" className="cta-btn-secondary" onClick={scrollToLeads}>
                  Match tiers to buyable leads
                </button>
              </div>
            </div>

            <div id="marketplace-leads" className="section-header-block">
        ...
        {activeTab === 'admin' && sessionUser?.role === 'admin' && (
          <section className="admin-portal fade-in glass">
            <div className="admin-portal-head">
              ...
              <p>
                Full ZIP visibility coast-to-coast, tiered list pricing, pilot quotas (per-metro visit targets — no longer
                limited to two states), and Slack/Discord ticket fan-out.
              </p>
            </div>
        ...
        {activeTab === 'flyover' && (
          <section className="flyover-section fade-in glass">
            <div className="flyover-header">
              ...
              <div className="tab-inline-cta flyover-cta glass-inset">
                <p>
                  <strong>Pair flyover with exclusivity:</strong> lock the same ZIPs you see on the live hail map — only here.
                </p>
                <button type="button" className="cta-btn-primary sm" onClick={scrollToLeads}>
                  Buy leads in storm corridors
                </button>
              </div>
            </div>
      ...
      <footer className="footer-bar glass">
        <div className="system-status">
          <span className="pulse" /> Live: Roof Hunter Pro · Ridgeline API{' '}
          {API_BASE || '(same-origin / GitHub Pages: set DASHBOARD_API_URL)'}
        </div>
        <div className="footer-cta-wrap">
          <button type="button" className="cta-btn-primary sm" onClick={scrollToLeads}>
            Get storm-matched leads
          </button>
        </div>
        <div className="user-info">
          <ShieldCheck size={14} /> Roof Hunter Pro · secured Ridgeline gateway
        </div>
      </footer>

      <SiteWideCtaBar onBrowseLeads={scrollToLeads} onSignIn={openSignIn} />
    </div>
  );
}
