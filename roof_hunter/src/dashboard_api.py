...
from ai_damage_engine import assessment_from_image  # noqa: E402
from nws_hail_alerts import get_hail_events, summarize_hail_alerts  # noqa: E402
from property_enrich import enrich_property  # noqa: E402
from call_agent_feed import normalize_call_agent_feed  # noqa: E402
...
class PredictDamageRequest(BaseModel):
    image_url: str = Field(..., min_length=1, description="Roof / property image URL for vision scoring (stub ignores content)")


class CallAgentFeedRequest(BaseModel):
    """Single-record feed for voice agents (Bland / ElevenLabs tooling)."""
    ...
    damage_score: float = Field(
        ...
        ge=0,
        le=100,
        description="Damage index: use 0–1 (e.g. 0.82 → 82%) or 0–100 percent",
    )
    priority: str = Field(..., min_length=1)
    script_type: str = Field(..., min_length=1, description="e.g. insurance_roof_claim")
    phone_number: Optional[str] = Field(None, description="E.164 — required if dispatch=true")
    dispatch: bool = Field(False, description="If true, POST to Bland /v1/calls when env + phone are set")
...
async def dispatch_elevenlabs_batch(csv_path: str = Body(..., embed=True)):
    ...
    try:
        ...
        results = []
        for lead in leads:
            phone = _row_phone(lead)
            if not phone:
                logger.warning("Skipping lead row with no phone: %s", lead)
                continue

            # Determine Regional Line (Telnyx Connection)
            # OK: +14052991068, TX: +19458370141
            state_val = lead.get("State") or lead.get("state") or ""
            is_oklahoma = "OK" in str(state_val).upper()
            caller_id_id = "phnum_5201kqe8agt5ffj8qf81ydk7ek54" if is_oklahoma else "phnum_0001kqe9hv3metra8y3ykgzg2arq"

            first_name = lead.get("First_Name") or lead.get("first_name") or "Homeowner"
            prop_addr = lead.get("Street Address") or lead.get("property_address") or ""
            city = lead.get("City") or lead.get("city") or ""
            hail_date = (
                lead.get("Event Date")
                or lead.get("hail_date")
                or "the recent storm"
            )
            damage_prob = (
                lead.get("AI Damage Score %")
                or lead.get("damage_probability")
                or ""
            )

            # Prepare conversation metadata for SIP Trunking (matches convai outbound-call shape)
            payload = {
                "agent_id": AGENT_ID,
            ...
            try:
                ...
                sms_message = f"Hi {first_name}, this is Sarah with Ridgeline. Our forensic scan flagged your property at {prop_addr} for hail damage. I'm trying to reach you to schedule a free verification scan. Call or text me back here!"
...
@app.post("/property")
def get_property(req: PropertyIntelRequest):
    """Stub geocode + roof estimate + enrichment (replace with real providers)."""
    enrichment = enrich_property(req.address)
    return {
        "address": req.address,
        "lat": 35.4676,
        "lng": -97.5164,
...
@app.post("/property/enrich")
def post_property_enrich(req: PropertyIntelRequest):
    """Owner/value/roof age/lead score stub keyed by address string."""
    return enrich_property(req.address)


@app.get("/imagery")
def get_imagery(lat: float, lng: float):
    """Tile URL template for map clients; swap for your provider (Mapbox, Planet, etc.)."""
    return {
...
def damage_score(property_id: str, image_url: Optional[str] = None):
    """Score from stub vision engine; pass `image_url` to key the placeholder model."""
    key = image_url.strip() if image_url and image_url.strip() else f"property:{property_id}"
    a = assessment_from_image(key)
    return {
        "property_id": property_id,
        "image_url": image_url,
        "hail_damage": a["hail_damage"],
        "missing_shingles": a["missing_shingles"],
...
def damage_predict(body: PredictDamageRequest):
    """Run the AI damage engine on an image URL (stub — does not download the image)."""
    a = assessment_from_image(body.image_url)
    return {
...
@app.post("/api/call-agent/feed")
def post_call_agent_feed(body: CallAgentFeedRequest):
    """
    Accept a lead payload and return Bland ``request_data`` + ElevenLabs-style dynamic variables.

    Optional: set ``dispatch=true`` and ``phone_number`` (+ ``BLAND_AI_API_KEY`` / ``BLAND_AGENT_ID``)
    to start a Bland call. Optional env ``BLAND_CALL_TASK`` adds a task string to the payload.
    """
    payload = body.model_dump()
    shapes = normalize_call_agent_feed(payload)
    out: dict = {
        "status": "accepted",
        "feed": {k: v for k, v in payload.items() if k != "dispatch"},
        **shapes,
        "bland_dispatch": None,
    }

    logger.info(
        "Call agent feed: name=%s script_type=%s priority=%s dispatch=%s",
        body.name,
        body.script_type,
        body.priority,
        body.dispatch,
    ...
    key = os.getenv("BLAND_AI_API_KEY")
    agent = os.getenv("BLAND_AGENT_ID")
    phone = (body.phone_number or "").strip()
    if not key or not agent:
        out["bland_dispatch"] = {
            "attempted": False,
            "error": "Set BLAND_AI_API_KEY and BLAND_AGENT_ID to dispatch",
        }
        return out
    if not phone:
        out["bland_dispatch"] = {"attempted": False, "error": "phone_number required when dispatch=true"}
        return out
    ...
    try:
        bland_payload: dict = {
            "phone_number": phone,
            "agent_id": agent,
            "request_data": shapes["bland_request_data"],
        }
        task = os.getenv("BLAND_CALL_TASK", "").strip()
        if task:
            bland_payload["task"] = task
        r = requests.post(
        ...
        out["bland_dispatch"] = {
            ...
            "body": (r.json() if ct.startswith("application/json") else r.text[:800]),
        }
    except Exception as e:
        logger.exception("Bland dispatch from call-agent feed failed")
        out["bland_dispatch"] = {"attempted": True, "error": str(e)[:400]}
...
async def api_damage_tiers(state: Optional[str] = None):
    nws_payload: dict = {"ok": False, "count": 0, "alerts": [], "error": None}
    try:
        raw = get_hail_events()
        nws_payload["ok"] = True
        nws_payload["count"] = len(raw)
        nws_payload["alerts"] = summarize_hail_alerts(raw)
    except Exception as e:
        logger.warning("NWS active hail alerts fetch failed: %s", e)
        nws_payload["error"] = str(e)[:240]
    ...
    h, m, l, n = (
        ...
        int(summary["medium"]),
        int(summary["low"]),
        int(nws_payload["count"]),
    )
    denom = max(1, h + m + l + n)
    lead_storm_pie = {
        "slices": [
            {"key": "lead_high", "label": "Leads · high (≥40)", "value": h, "pct": round(100.0 * h / denom, 1)},
            {"key": "lead_medium", "label": "Leads · medium", "value": m, "pct": round(100.0 * m / denom, 1)},
            {"key": "lead_low", "label": "Leads · low", "value": l, "pct": round(100.0 * l / denom, 1)},
            {"key": "nws_hail", "label": "NWS active alerts (hail mention)", "value": n, "pct": round(100.0 * n / denom, 1)},
        ],
        "denominator_note": "Percents share one bar: lead tiers + count of active alerts mentioning hail.",
    }
    return {
        "filter_state": state,
        "summary": summary,
        "by_state": by_state,
        "nws_hail_alerts": nws_payload,
        "lead_storm_pie": lead_storm_pie,
    }
