...
Run:
  ...
  python3 src/live_hail_ingest.py --mesh       # MRMS MESH 24h grid (needs requirements-radar.txt)
"""
...
def fetch_spc_live(days=7, min_size_inches=1.0):
    """Fetch SPC hail/wind reports for last N days. Each row gets the archive-day date (UTC calendar day for that file)."""
    now = datetime.now(timezone.utc)
    today = now.strftime("%Y-%m-%d")
    yesterday = (now - timedelta(days=1)).strftime("%Y-%m-%d")

    batches = []
    batches.append((fetch_spc_csv(SPC_TODAY_HAIL, "today_hail"), today))
    batches.append((fetch_spc_csv(SPC_YESTERDAY_HAIL, "yesterday_hail"), yesterday))
    batches.append((fetch_spc_csv(SPC_TODAY_WIND, "today_wind"), today))
    batches.append((fetch_spc_csv(SPC_YESTERDAY_WIND, "yesterday_wind"), yesterday))
    ...
    for day_offset in range(2, days):
        ...
        ed = d.strftime("%Y-%m-%d")
        batches.append(
            (fetch_spc_csv(SPC_ARCHIVE_HAIL.format(date=date_str), f"{ed}_hail"), ed)
        )
        batches.append(
            (fetch_spc_csv(SPC_ARCHIVE_WIND.format(date=date_str), f"{ed}_wind"), ed)
        )

    total_raw = sum(len(b[0]) for b in batches)
    print(f"  [SPC] Total raw reports across {days} days: {total_raw}")

    normalized = []
    for raw_rows, event_date in batches:
        for row in raw_rows:
            try:
                # Wind CSV uses Speed; hail CSV uses Size (column sets do not overlap)
                is_wind = "Speed" in row and "Size" not in row
                size_raw = float(row.get("Size", row.get("Speed", 0)))

                if is_wind:
                    # SPC documents gusts in MPH on the reports page; keep strong wind only
                    if size_raw < 60:
                        continue
                    size_in = size_raw
                    event_type = "Wind"
...
def ingest_spc(days=7, min_size=1.0):
    ...
    for i, hit in enumerate(hits):
        ...
        event_type = hit.get("event_type", "Hail")
        ...
        unit = "mph wind" if event_type == "Wind" else '" hail'
        print(f"  [{i+1}/{total}] {magnitude}{unit} | {location}, {hit['state_abbr']} | ({lat:.3f},{lon:.3f}) | {event_date}")
        ...
        insert_storm(conn, {
            ...
            "hail_prob":  100,
            "location_label": (
                f'{magnitude:g} mph wind — {location}, {hit["state_abbr"]}'
                if event_type == "Wind"
                else f'{magnitude}" hail — {location}, {hit["state_abbr"]}'
            ),
        })
        inserted += 1
...
def prune_storms_before_date(conn, cutoff_date: str) -> int:
    """Delete storms with event_date strictly before cutoff_date (YYYY-MM-DD)."""
    c = conn.cursor()
    c.execute("DELETE FROM storms WHERE event_date < ?", (cutoff_date,))
    n = c.rowcount or 0
    conn.commit()
    return n
...
def print_summary():
    ...
    manual = c.fetchone()[0]
    c.execute("SELECT COUNT(*) FROM storms WHERE source='NOAA_MRMS_MESH'")
    mesh = c.fetchone()[0]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="If >0, delete storms older than this many days (rolling retention). 0 = keep all.",
    )
    parser.add_argument(
        "--mesh",
        action="store_true",
        help="Ingest NOAA MRMS MESH 24h-max hail grid (pip install -r requirements-radar.txt).",
    )
    parser.add_argument("--mesh-min-inches", type=float, default=1.0, help="MRMS peak filter (inches)")
    parser.add_argument("--mesh-max-events", type=int, default=400, help="Cap MRMS rows per run")
    parser.add_argument("--mesh-geocode", type=int, default=0, help="Geocode first N MRMS peaks (slow)")
    ...
    if not args.manual:
        print(f"\n[1/3] Fetching NOAA SPC live hail — last {args.days} days (≥{args.min}\")")
        ingest_spc(days=args.days, min_size=args.min)
    else:
        print("\n[MANUAL MODE] Skipping SPC fetch")

    if args.mesh:
        print("\n[MRMS] Ingesting NOAA MRMS MESH 24h-max grid…")
        try:
            from mrms_mesh_ingest import ingest_mrms_mesh

            s = ingest_mrms_mesh(
                db_path=DB_PATH,
                min_inches=args.mesh_min_inches,
                max_events=args.mesh_max_events,
                ge_codes=args.mesh_geocode,
            )
            print(
                f"  MRMS peaks={s['peaks']} inserted={s['inserted']} "
                f"skipped/dup={s['skipped']} ({s['event_date']})"
            )
        except ImportError as e:
            print("  [MRMS SKIP] Install radar stack: pip install -r requirements-radar.txt\n ", e)
        except Exception as e:  # noqa: BLE001
            print("  [MRMS FAIL]", e)

    print("\n[field reports] Injecting Oklahoma field sites:")
    for s in MANUAL_SITES:
        print(f"  • {s['label']} ({s['magnitude']}\")")
    inject_manual_sites()

    if args.prune_days > 0:
        co = (datetime.now(timezone.utc) - timedelta(days=args.prune_days)).strftime("%Y-%m-%d")
        conn = get_db()
        n = prune_storms_before_date(conn, co)
        conn.close()
        print(f"\n[PRUNE] Removed {n} rows with event_date before {co} (keep last {args.prune_days} days)")
