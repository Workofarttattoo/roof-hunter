...
@dataclass
class PortfolioOptimizationMarkowitzConfig:
    """Configuration for Portfolio Optimization Markowitz"""
    iterations: int = 1000
    precision: float = 1e-6

class PortfolioOptimizationMarkowitzLab:
    """
    Portfolio Optimization Markowitz
    ...
    def __init__(self, config: PortfolioOptimizationMarkowitzConfig):
        self.config = config
        self.results = []
...
def demo():
    """Demo of Portfolio Optimization Markowitz"""
    print(f"{lab_name} - Built by ECH0")
    print("="*60)

    config = PortfolioOptimizationMarkowitzConfig(iterations=500)
    lab = PortfolioOptimizationMarkowitzLab(config)
